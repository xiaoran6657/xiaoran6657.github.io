...
export const cases: Case[] = [
  ...
  },
  {
    ...
    approach: { zh: '由 Agent 协助准备、实现与审查，确定性 C# Runner 执行采样，再对导出结果做验证。运行终态（如 Completed / Failed）、正确性与测量有效性分别记录，退出码和性能判定也单独处理。', en: 'Agents assist preparation, implementation, and review; a deterministic C# Runner samples data before post-export validation. Terminal state (such as Completed / Failed), correctness, and measurement validity are separate fields; exit codes and performance judgments are also handled separately.' },
